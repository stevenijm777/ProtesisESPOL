...
QueueHandle_t stopQueue;  // Cola para comunicación de detención

// Pin del sensor de fuerza
const int forceSensorPin = A0; // Ajustar según el pin usado para el sensor
...
// Tarea para monitorear el sensor de fuerza y detener el servo
void monitorForceSensor(void *pvParameters) {
    const int forceThreshold = 500;  // Ajustar según el umbral adecuado para el sensor

    while (1) {
        int forceValue = analogRead(forceSensorPin);  // Leer valor del sensor de fuerza
        if (forceValue > forceThreshold) {
            // Enviar señal de detención a la cola
            bool stopSignal = true;
            xQueueSend(stopQueue, &stopSignal, portMAX_DELAY);

            // Esperar 2 segundos antes de continuar
            delay(2000);
        }
        delay(100);  // Ajustar frecuencia de lectura según necesidad
    }
}

// Tarea para detener el servo cuando se reciba una señal de detención
void stopServo(void *pvParameters) {
    bool stopSignal;

    while (1) {
        if (xQueueReceive(stopQueue, &stopSignal, portMAX_DELAY) == pdTRUE) {
            if (stopSignal) {
                myservo.write(90);  // Detener el servo (ajustar según sea necesario)
                delay(2000);  // Esperar 2 segundos antes de permitir movimiento
            }
        }
    }
}
...
void setup() {
    ...
    pinMode(forceSensorPin, INPUT);  // Configura el pin del sensor de fuerza como entrada
    Serial.begin(115200);  // Inicializa la comunicación serie para debug
    initModel();  // Inicializar el modelo TFLite

    // Crear las colas para comunicación entre tareas
    dataQueue = xQueueCreate(10, sizeof(float) * 3);
    stopQueue = xQueueCreate(5, sizeof(bool));
    ...
    xTaskCreatePinnedToCore(monitorForceSensor, "MonitorForceSensor", 2048, NULL, 1, NULL, 0);
    xTaskCreatePinnedToCore(stopServo, "StopServo", 2048, NULL, 1, NULL, 1);
}
