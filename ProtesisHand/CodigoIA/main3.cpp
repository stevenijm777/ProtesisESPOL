...
int sensorThreshold = 3000; // Umbral del sensor de fuerza para detener el movimiento
bool movimientoEnCurso = false; // Bandera para indicar si hay un movimiento en curso

int posicionActualPinza = 0;      // Variable global para almacenar la posición actual del servoPinza
int posicionActualAbreCierra = 0; // Variable global para almacenar la posición actual del servoAbreCierra

// Pines de los interruptores
const int pinManualAuto = 16;
const int pinAbreCierra = 17;
const int pinPinza = 18;
const int pinBloqueo = 19;
...
void controlarServoPinza() {
    ...
    while (true) {
        for (int pos = 0; pos <= 180; pos += 10) {
            if (analogRead(35) > sensorThreshold) {
                posicionActualPinza = pos;  // Guardar la posición actual
                movimientoEnCurso = false;
                return;  // Detener el movimiento sin regresar
            }
...
void controlarServoAbreCierra() {
    ...
    while (true) {
        for (int pos = 0; pos <= 180; pos += 10) {
            if (analogRead(35) > sensorThreshold) {
                posicionActualPinza = pos;  // Guardar la posición actual
                posicionActualAbreCierra = pos;  // Guardar la posición actual
                movimientoEnCurso = false;
                return;  // Detener el movimiento sin regresar
            }
            servoPinza.write(pos);
...
// Función para regresar el servo a su posición original
void regresarServoPinza() {
    movimientoEnCurso = true;
    for (int pos = posicionActualPinza; pos >= 0; pos -= 10) {
        servoPinza.write(pos);
        delay(200);
    }
    posicionActualPinza = 0;  // Actualizar la posición actual a 0
    movimientoEnCurso = false;
}

void regresarServoAbreCierra() {
    movimientoEnCurso = true;
    for (int pos = posicionActualAbreCierra; pos >= 0; pos -= 10) {
        servoPinza.write(pos);
        servoAbreCierra.write(pos);
        delay(200);
    }
    posicionActualPinza = 0;  // Actualizar la posición actual a 0
    posicionActualAbreCierra = 0;  // Actualizar la posición actual a 0
    movimientoEnCurso = false;
}
...
void controlarServos(int clase) {
    ...
    switch (clase) {
        ...
        case 4: // Descanso
            ...
            break;
        case 5: // Descanso2
            regresarServoPinza();  // Regresar el servo a su posición original
            regresarServoAbreCierra();
            break;
...
void setup() {
    ...
    pinMode(pinManualAuto, INPUT_PULLUP);
    pinMode(pinAbreCierra, INPUT_PULLDOWN);
    pinMode(pinPinza, INPUT_PULLDOWN);
    pinMode(pinBloqueo, INPUT_PULLUP);
...
void loop() {
    ...
    modoAutomatico = digitalRead(pinManualAuto);
    bool abreCierra = digitalRead(pinAbreCierra);
    bool pinza = digitalRead(pinPinza);
    bloqueoActivado = digitalRead(pinBloqueo);

    // Imprimir el estado de los interruptores y el modo en que se encuentra el sistema
    Serial.println("=== Estado del Sistema ===");
    Serial.print("Modo: ");
    Serial.println(modoAutomatico ? "Automático" : "Manual");
    Serial.print("Estado del Pin Abre/Cierra: ");
    Serial.println(abreCierra ? "Activo" : "Inactivo");
    Serial.print("Estado del Pin Pinza: ");
    Serial.println(pinza ? "Activo" : "Inactivo");
    Serial.print("Estado del Pin Bloqueo: ");
    Serial.println(bloqueoActivado ? "Activado" : "Desactivado");
    Serial.println("==========================");
    ...
    if (modoAutomatico) {
        ...
        if (bloqueoActivado) {
            // Si el bloqueo está activado, no hacer nada
            Serial.println("Bloqueo activado en modo automático. Motores detenidos.");
        } else {
            // Continuar con el funcionamiento automático normal
            // Las tareas de predicción ya están corriendo en modo automático
            Serial.println("Modo automático sin bloqueo. Funcionamiento normal.");
        }
    } else {
        // Modo manual
        Serial.println("Modo Manual Activado");
        if (abreCierra) {
            Serial.println("Abre y Cierra Activado");
            controlarServoAbreCierra();
        } else {
            Serial.println("Abre y Cierra Inactivo. Regresando motores a su lugar.");
            regresarServoAbreCierra(); // Regresa los motores a su lugar si no está en "Abre y Cierra"
        }

        if (pinza) {
            Serial.println("Pinza Activado");
            controlarServoPinza();
        } else {
            Serial.println("Pinza Inactivo. Regresando motor a su lugar.");
            regresarServoPinza(); // Regresa los motores a su lugar si no está en "Pinza"
        }
    }

    delay(100); // Pequeño retraso para evitar lecturas erróneas
}