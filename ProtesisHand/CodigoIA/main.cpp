...
//#include "modelo_entrenado_relu2.h"  // Usar el nuevo modelo entrenado
#include "modelo_entrenado_relu1.h"
...
int predict(float dato) {
    ...
    // Imprimir el valor predicho en el monitor serial
...
void readSensorData(void *pvParameters) {
    ...
    while (1) {
        ...
        int mapeadoValue = map(sensorValue, 0, 4095, 0, 1023);        // Imprimir el valor mapeado para depuración
        //Serial.print("Valor del sensor (mapeado): ");
        Serial.println(mapeadoValue);
...
void procesarPredicciones() {
    ...
    int maxConteo = 0;
    const int pesoAbreYCierra = 3;  // Peso adicional para la clase "Abre y Cierra"
    const int pesoPinza = 10;        // Peso adicional para la clase "Pinza"

    for (int i = 0; i < 9; i++) {  // Ajustado para 9 clases
    ...
    for (int i = 0; i < 9; i++) {  // Ajustado para 5 clases
        predicciones[i] = 0;
    }
}
...
void predictAndMoveServo(void *pvParameters) {
    ...
    while (1) {
        if (xQueueReceive(dataQueue, &datos_procesados, portMAX_DELAY) == pdTRUE) {
            ...
            if (clase >= 0 && clase < 9) {  // Ajustado para 5 clases
                predicciones[clase]++;
            }
